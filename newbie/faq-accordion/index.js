...
titles.forEach((title) => {
    title.addEventListener('click', function() {
        const activeItem = document.querySelector('.fac-container .ativo');
        const answer = this.nextElementSibling;
        const svg = this.querySelector('.svg')

        if (activeItem && activeItem !== answer) {
            activeItem.classList.remove('ativo');
        }

        answer.classList.toggle('ativo');

        if (answer.classList.contains('ativo')) {
            svg.innerHTML = `
